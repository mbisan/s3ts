...
def pretrain_figure_vertical(df: pd.DataFrame,
        fontsize: int = 18,
        metric: str = "test_acc",
        metric_lab: str = "Test Accuracy",
        fname: str = "figures/ablation/pretrainv.pdf"
        ) -> pd.DataFrame:
...
def pretrain_figure_horizontal(df: pd.DataFrame,
        fontsize: int = 18,
        metric: str = "test_acc",
        metric_lab: str = "Test Accuracy",
        fname: str = "figures/ablation/pretrainh.pdf"
        ) -> pd.DataFrame:
    ...
    g: sns.FacetGrid = sns.relplot(data=data, x="train_exc_limit", y=metric, hue='Arch', 
        kind="line", col="dataset", row="mode", palette=sns.color_palette("bright"),
        height=2.5, aspect=1, legend="brief", markers="True", row_order=["DF", "GAF"], 
        col_order=["ArrowHead", "CBF", "ECG200", "GunPoint", "SyntheticControl", "Trace"],
        facet_kws={"despine": False, "margin_titles": True, "legend_out": True,
                   "sharey": False, "sharex": True})
    ...
    #    ylim=[15, 80], yticks=[20, 30, 40, 50, 60, 70])
    g.figure.subplots_adjust(wspace=0, hspace=0)
    g.legend.__setattr__("fontsize", fontsize)
    g.legend.set_title("")


    dsets = ["ArrowHead", "CBF", "ECG200", "GunPoint", "SyntheticControl", "Trace"]
    ybound = ([25, 85], [30, 40, 50, 60, 70, 80])
