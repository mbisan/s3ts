...
def EXP_wtst(
    ...
    for i, wlen in enumerate(WINDOW_LENGTHS):
        ...
        for j, wts in enumerate(TIME_STRIDES):

            # Update the data module time stride
            dm.update_properties(window_time_stride=wts)

            # Run the model without pretraining
            data, _ = train_model(
                label=f"{i}{j}_wdw{wlen}_wts{wts}",
            ...
            # Update the data module to series stride
            dm.update_properties(stride_series=True)

            # Run the model with pretraining 
            data, _ = train_model(
                label=f"{i}{j}_wdw{wlen}_wts{wts}_pre_str",
            ...
            # Run the model with pretraining 
            data, _ = train_model(
                label=f"{i}{j}_wdw{wlen}_wts{wts}_pre_nstr",
                dataset=dataset, repr=repr, arch=arch, dm=dm, pretrain=True, 
                fold_number=fold_number, directory=subdir_train,
                max_epoch_pre = max_epoch_pre, max_epoch_tgt=max_epoch_tra,
                learning_rate=learning_rate, random_state=random_state,
