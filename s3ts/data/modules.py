...
class FullDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[np.ndarray, int]:
        ...
        frame = self.frames[:,:,idx - self.window_length*self.window_stride+1:idx+1:self.window_stride]
        if self.frames_transform:
            frame = self.frames_transform(frame)

        series = self.series_tensor[0,0,idx - self.window_length*self.window_stride+1:idx+1:self.window_stride]
